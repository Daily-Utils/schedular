...
export class SupportTicketsService {
    ...
    async getAllSupportTickets(paitent_user_id: number){
        return await this.supportTicketsRepository.find({where: {patient_user_id: paitent_user_id}});
    }

    async updateSupportTicketStatus(updateSupportDTO: UpdateSupportDTO){
        return await this.supportTicketsRepository.update(updateSupportDTO.patient_user_id, updateSupportDTO);
    }
